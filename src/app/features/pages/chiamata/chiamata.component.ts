...
export class ChiamataComponent implements OnInit, OnChanges, GridOptions {
  ...
  getAll(params) {
    ...
    return this.http.get('http://localhost:8080/cliente/all1', {params}).subscribe(
      risp => {
        //   console.log("questo è ", risp)
        const keys = Object.keys(risp[0]);
        const columnDefs = [];
        // columnDefs.push({ width: 50, cellRenderer: 'costumCell' });
        columnDefs.push({width: 50, template: '<img src="assets/images/dettaglio.gif"   >'});
        for (const k of keys) {
          columnDefs.push({
            headerName: k,
            field: k,
